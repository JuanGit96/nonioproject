...
class SimulationController extends Controller
{
    ...
    public function createSimulation(Request $request)
    {
        ...
        if ($User == 1) {
            ...
            if($simulationAux = Simulation::where('email_contact',$request->email_contact)->get()){
                foreach($simulationAux as $simAux){
                    if($capacityAux = IndebtednessCapacity::where('simulation_id',$simAux->id)->get()){
                        foreach($capacityAux as $capAux){
                            $capAux->delete();
                        }
                    }
                    $simAux->delete();
        ...
        if ($simulation->save()) {
            ...
            return redirect()->route('accepted',['id' => $simulation->id]);
        }else {
            # code...
            return false;
        }
    ...
    {
	//$simulation = Simulation::where('email_contact',$request->email_contact)->count();
	$simulation = User::where('email',$request->email_contact)->count();
